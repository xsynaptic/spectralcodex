...
Install the git hooks once with `pnpm exec lefthook install`. The pre-push hook runs the `check` group defined in `lefthook.yml`, the repository's quality gate (stylelint, prettier, eslint, `astro check`, knip, fallow, vitest), which can also be run standalone as `pnpm check`.
