...
const getOpenGraphImageFontData = (filename: string) => {
	try {
		...
		throw new Error(`Error reading font files for open graph image generation: ${String(error)}`);
	}
};

// Ensure fonts loaded here are already present in `public/fonts`
// This is so we have the option to generate OG images on the server
